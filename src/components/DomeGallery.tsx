...
type DomeGalleryProps = {
  ...
  enlargePosition?: 'center' | 'left' | 'right';
};
...
const CUSTOM_SUBTITLES: Record<string, string> = {
  '1-jeremy-michaiah-dr-e': 'RLST 184 - Spring 2016',
  'lpa': 'Latino Pentecostal Identity',
  'pos': 'Prophets of the Spirit',
  'rrbo': 'Religion and the Republic',
  'rrap': 'Religion, Race, and the American Presidency',
  // Add as many as you need here!
};

// Automatically import all images from your homepage folder
const imageModules = import.meta.glob('../assets/homepage/*.{png,jpg,jpeg,webp,PNG,JPG,JPEG,WEBP}', { eager: true });
...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  if (pool.length === 0) {
    return coords.map(c => ({ ...c, src: '', alt: '', subtitle: '' }));
  }

  const normalizedImages = pool.map(image => {
    if (typeof image === 'string') {
      return { src: image, alt: '', subtitle: '' };
    }
    return { src: image.src || '', alt: image.alt || '', subtitle: image.subtitle || '' };
  });
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (!draggingRef.current && !inertiaRAF.current && !focusedElRef.current && !openingRef.current) {
        const nextY = wrapAngleSigned(rotationRef.current.y + autoRotateSpeed * (delta / 16.66));
    ...
    autoRotateRAF.current = requestAnimationFrame((time) => {
      lastTime = time;
      loop(time);
    });
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    const rawSubtitle = parent.dataset.subtitle || '';
  ...
  let viewerJustifyClass = 'justify-center';
  if (enlargePosition === 'left') viewerJustifyClass = 'justify-start';
  if (enlargePosition === 'right') viewerJustifyClass = 'justify-end';
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div
            ref={viewerRef}
            className={`absolute inset-0 z-20 pointer-events-none flex items-center ${viewerJustifyClass}`}
